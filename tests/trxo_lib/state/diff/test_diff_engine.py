...
from trxo_lib.state.diff.diff_engine import ChangeType, DiffEngine


def _mock_insights(mocker):
    mocker.patch(
        "trxo_lib.state.diff.insights_generator.InsightsGenerator.generate_key_insights",
        return_value=[],
    )
...
def test_compare_data_added_item(mocker):
    ...
    mocker.patch("trxo_lib.state.diff.diff_engine.DeepDiff", return_value={})
...
def test_compare_data_modified_item(mocker):
    ...
    mocker.patch("trxo_lib.state.diff.diff_engine.DeepDiff", return_value=diff_obj)
    _mock_insights(mocker)
...
def test_extract_items_from_command_name_key():
    engine = DiffEngine()

    data = {"nodes": {"1": {"_id": "1", "name": "A"}}}
