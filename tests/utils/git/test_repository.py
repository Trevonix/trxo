...
def test_init_empty_repo_happy_path(tmp_path, mocker):
    repo = make_repo_mock()
    mocker.patch("trxo_lib.git.repository.Repo.init", return_value=repo)
...
def test_clone_or_init_repo_clone_success(tmp_path, mocker):
    repo = make_repo_mock()
    mocker.patch("trxo_lib.git.repository.Repo.clone_from", return_value=repo)
...
def test_clone_or_init_repo_empty_remote_fallback(tmp_path, mocker):
    mocker.patch(
        "trxo_lib.git.repository.Repo.clone_from",
        side_effect=GitCommandError("x", 1, "remote HEAD refers to nonexistent ref"),
    )

    init_repo = make_repo_mock()
    mocker.patch("trxo_lib.git.repository.init_empty_repo", return_value=init_repo)
...
def test_get_or_create_repo_existing_valid_repo(tmp_path, mocker):
    ...
    repo = make_repo_mock()
    mocker.patch("trxo_lib.git.repository.Repo", return_value=repo)
...
def test_get_or_create_repo_existing_invalid_repo(tmp_path, mocker):
    ...
    mocker.patch(
        "trxo_lib.git.repository.Repo", side_effect=InvalidGitRepositoryError
    )

    clone_repo = make_repo_mock()
    mocker.patch(
        "trxo_lib.git.repository.clone_or_init_repo", return_value=clone_repo
    )
