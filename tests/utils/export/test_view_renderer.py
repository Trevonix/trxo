...
from trxo_lib.utils.export.view_renderer import ViewRenderer
...
def test_create_table_no_items(mocker):
    mocker.patch("trxo_lib.utils.export.view_renderer.info")
...
def test_create_table_valid(mocker):
    mock_print = mocker.patch("trxo_lib.utils.export.view_renderer.console.print")
    mocker.patch("trxo_lib.utils.export.view_renderer.info")
    items = [{"a": 1, "b": "x"}]
...
def test_create_table_with_selected_columns(mocker):
    mocker.patch("trxo_lib.utils.export.view_renderer.console.print")
    mocker.patch("trxo_lib.utils.export.view_renderer.info")
    items = [{"a": 1, "b": 2}]
...
def test_create_table_with_invalid_selected_columns(mocker):
    mocker.patch("trxo_lib.utils.export.view_renderer.error")
    mocker.patch("trxo_lib.utils.export.view_renderer.info")
...
def test_display_nested_structure_nested_tables(mocker):
    mocker.patch("trxo_lib.utils.export.view_renderer.ViewRenderer.create_table")
    data = {"x": {"y": [{"a": 1, "b": 2}]}}
    ViewRenderer.display_nested_structure(data, "scripts", None)


def test_display_nested_structure_fallback_single_object(mocker):
    mocker.patch("trxo_lib.utils.export.view_renderer.ViewRenderer.display_single_object")
    data = {"a": 1}
    ViewRenderer.display_nested_structure(data, "scripts", None)
...
def test_display_table_view_nested_dict(mocker):
    mocker.patch(
        "trxo_lib.utils.export.view_renderer.ViewRenderer.display_nested_structure"
    )
