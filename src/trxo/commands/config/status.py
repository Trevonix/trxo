...
from trxo_lib.git.credentials import validate_credentials
...
class StatusChecker:
    ...
    def run(self) -> None:
        ...
        if self.am_configured:
            self.perform_am_checks()

        if self.idm_configured:
    ...
    def detect_auth_mode(self) -> None:
        ...
        auth_mode = self.config.get("auth_mode", "service-account")
        ...
        if auth_mode == "onprem":
            # In on-prem mode, base_url is often a duplicate of am_base_url
            self.aic_url = None
        else:
            self.aic_url = (
